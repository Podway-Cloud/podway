# postgres-pod-store Specification

## Purpose
Provides durable, owner-scoped persistence for pods via a migrated Postgres table and a Drizzle-backed implementation of the pod store contract (create, owner-scoped list, read, update, and delete). It exists so PodService's launch and lifecycle operations run unchanged against a real database.
## Requirements
