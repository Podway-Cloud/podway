...
## Purpose
Provides the browser-based terminal that connects to a pod, renders its PTY input and output, and sends resizes over a protocol client that is testable in isolation. It turns agent-output URLs into clickable chips (promoting OAuth links), bridges the clipboard, stays usable on mobile, manages connection status with reconnection, and requires an authenticated session to view.
## Requirements
